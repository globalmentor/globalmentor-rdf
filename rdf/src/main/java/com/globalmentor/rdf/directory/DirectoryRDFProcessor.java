...
public class DirectoryRDFProcessor extends AbstractRDFProcessor {
	...
	protected PredefinedRDFProfile getPredefinedProfile() {
	...
	/**
	 * Processes a directory and converts content lines into property of the given RDF resource.
	 * <p>
	 * For each content line, an RDF property resource and an RDF value object are created and added to the resource as a property. The property and value are
	 * created using factories obtained in the following manner:
	 * </p>
	 * <ol>
	 * <li>If a profile is registered that implements <code>RDFPropertyFactory</code>, it is used to create the property. Otherwise, the predefined profile is
	 * asked to create the property.</li>
	 * <li>If the content line doesn't specifiy a value type, but the profile implements <code>ValueFactory</code>, that profile is asked for the value type.</li>
	 * <li>If the profile implements <code>RDFPropertyValueFactory</code>, the profile is asked to create the value.</li>
	 * <li>If there is a value type and there is a property value factory specifically registered for that value type, it is asked to create the value.</li>
	 * <li>If no RDF property value object can be created, the content line is ignored.</li>
	 * </ol>
			...
			if(property != null) { //if we have a property
				...
				if(valueType == null) { //if the value type wasn't explicitly given
					...
					if(valueType == null && profile != getPredefinedProfile()) { //if we still don't know the type, and we didn't already check the predefined profile 
						valueType = getPredefinedProfile()
								.getValueType(contentLine.getProfile(), contentLine.getGroup(), contentLine.getName(), contentLine.getParamList()); //ask the predefined profile for the value type
					}
				}
