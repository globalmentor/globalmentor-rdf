 ...
 * throughout its lifetime that is continually updated with every new RDF processing that occurs.
 * <p>
 * The RDF processor maintains RDF data in two separate formats: the RDF data model <code>RDF</code>, as well as a list of statements used to create the data
 * model. The RDF data model may be replaced and its members modified, but these actions will not update the list of RDF statements. The RDF statements are only
 * generated by the RDF processor itself as it parses RDF serializations, and are available to give information on the parser actions.
 * </p>
 * TODO If a property from a non-RDF namespace has (for example) a "resource" attribute (i.e. a property with no namespace), property resources are not
 * correctly created and can cause endless loops when trying to analyze the namespace
...
public class RDFXMLProcessor extends AbstractRDFProcessor {
	...
	public enum NamespaceRequirement {
		...
		/**
		 * The RDF namespace or the <code>null</code> namespace is accepted for an RDF attribute. The <code>null</code> namespace is only accepted for attributes if
		 * the parent element is in the RDF namespace.
		 */
		RDF_OR_NULL,

		/**
		 * Any namespace is accepted for an RDF attribute. This is useful, for example, as a stop-gap measure for systems that corrupt attribute namespaces, such as
		 * Apache httpd 2.2.3 mod_dav. This setting is dangerous, however, as any RDF properties stored as attributes with the same local name as an RDF attribute
		 * (e.g. <code>about</code> and <code>first</code> are essentially ignored).
		 */
		ANY
	};
	...
	public NamespaceRequirement getRDFAttributeNamespaceRequirement() {
		return rdfAttributeNamespaceRequirement;
	}
	...
	/**
	 * Processes RDF serialized in an XML document. Processes data contained in every <code>&lt;rdf:RDF&gt;</code> data island.
	 * @param document The XML document that might contain RDF data.
	 * @param baseURI The base URI, or <code>null</code> if the base URI is not known.
	...
	/**
	 * Processes RDF serialized in an XML document. Processes data contained in every <code>&lt;rdf:RDF&gt;</code> data island. Whatever base URI has been set is
	 * unchanged.
	 * @param document The XML document that might contain RDF data.
	 * @return The RDF data model resulting from this processing and any previous processing.
	 * @throws URISyntaxException Thrown if a URI is syntactically incorrect.
	...
	/**
	 * Processes RDF serialized in an XML document. Searches the given element and all its children, processing data contained in every
	...
	/**
	 * Processes RDF serialized in an XML document. Searches the given element and all its children, processing data contained in every
	 * <code>&lt;rdf:RDF&gt;</code> data island. Whatever base URI has been set is unchanged.
	 * @param element The XML element that might contain RDF data.
	 * @return The RDF data model resulting from this processing and any previous processing.
	 * @throws URISyntaxException Thrown if a URI is syntactically incorrect.
	 */
	...
	/**
	 * Processes an RDF resource serialized in the provided XML document. Whatever base URI has been set is unchanged.
	 * @param element The XML element that might contain RDF data.
	 * @return The RDF data model resulting from this processing and any previous processing.
	 * @throws URISyntaxException Thrown if a URI is syntactically incorrect.
	 */
	...
	/**
	 * Parses the attributes of the given element and assign them as properties to the given resource. Special RDF properties such as <code>rdf:about</code> are
	 * ignored.
	 * @param resource The object that represents the resource to which attribute properties should be added.
	 * @param element The element that contains the attributes to be considered properties.
	 * @param context Whether the attributes are part of a resource description, a resource reference, a reference to a blank node resource in short form, or the
	 *          creation of a blank node from an empty property element.
	...
	/**
	 * Processes the contents of a property element as normal property contents. Nodes that are not elements are not capable of determining RDF typed literal
	...
	/**
	 * Retrieves an RDF attribute from an element, if it exists, recognizing either prefixed or non-prefixed attributes. If the non-prefixed form is used, a
	 * warning is generated. It is assumed that this method will only be called once for a particular attribute, as each call could produce another warning.
	...
	/**
	 * Creates a URI by resolving the given URI relative to the base URI of the provided element, taking into account the given base URI of the document. URIs are
	 * resolved according to RDF/XML processing rules.
	...
	/**
	 * Resolves a URI against a base URI according to RDF/XML processing rules. Significantly, the emptry string is appended to the given base URI.
