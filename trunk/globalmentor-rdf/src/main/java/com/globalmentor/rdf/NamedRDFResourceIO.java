 ...
 * URI, resolved against the given base URI if one is available. Resolution of a URI against the base URI is performed according to <a
 * href="http://www.w3.org/TR/2003/PR-rdf-syntax-grammar-20031215/#section-baseURIs">RDF/XML Syntax Specification (Revised) 5.3 Resolving URIs</a>.
...
public class NamedRDFResourceIO<T extends RDFResource> extends AbstractRDFXMLIO<T> {
	 ...
	 * @throws NullPointerException if the given class and/or resource URI is <code>null</code>.
	 */
	...
	/**
	 * Reads a resource from an input stream using an existing RDF instance. If a base URI is given, the resource URI will be resolved against the base URI before
	 * it is used to determine the resource to return.
