 ...
 * with every new RDF processing that occurs.
 * <p>
 ...
 * </p>
 * <p>
 * An RDF processor takes the following steps:
 * </p>
 * <ol>
 * <li>All literals are created and stored in statements.</li>
 * <li>All typed resources are created and stored in the RDF data model and in statements.</li>
 * <li>After processing is ended, all untyped subjects and predicates that have not yet been created are created, preferably by using any type that has become
 * available through processing the RDF instance.</li>
...
public abstract class AbstractRDFProcessor {
	...
	/**
	 * The base URI of the RDF being processed, or <code>null</code> if no base URI was specified.
	 */
	...
	/**
	 * @return The base URI of the RDF being processed, or "online:" if no base URI is known.
	 */
	...
	/**
	 * Adds a statement to the list of statements. If an equivalent statement already exists in the list, no action occurs.
	...
	/**
	 * Resets the processor by clearing all temporary references and maps, such as associations between resource proxies and RDF resources. The set of statements
	 * is left undisturbed.
	 */
	public void reset() {
	...
	/**
	 * Iterates through all collected statements and, for any resources in the statement that are only proxies for RDF resources, creates appropriate resources,
	 * using any provided types in other statements if possible. The final created RDF resource of the given resource is returned.
	 * @param resource The resource, either an RDF resource or a proxy, for which an unproxied RDF resource should be returned, or <code>null</code> if no
	 *          specific resource should be returned.
	...
	/**
	 * For the given resource proxy, returns the existing RDF resource the proxy represents or, if there is no target, creates a new RDF resource for the proxy.
	 * If the latter, the statements are iterated to determine an appropriate type for the new resource, if possible.
	 * @param resourceProxy The resource that represents the RDF resource by reference URI or node ID.
	 * @return A
	 */
	...
	/**
	 * Processes all statements by creating associations between resources that reflect the assertions contained in the statements.
	 */
		...
		while(statementIterator.hasNext()) { //while there are more statements
			...
			if(subject instanceof RDFResource && predicate instanceof RDFResource) {
				...
				if(object instanceof RDFObject) { //if the object is an RDF object
					//TODO del Log.trace("ready to add predicate to subject:", rdfSubject);
	...
	/**
	 * A class which represents a resource using either a reference URI or a node ID, but not both.
	 * @author Garret Wilson
	 */
	protected static class ResourceProxy extends DefaultResource {
		...
		/**
		 * Full constructor.
		 * @param referenceURI The reference URI of the resource, or <code>null</code> if there is no reference URI but there is a node ID.
		 * @param nodeID The node ID of the resource, or <code>null</code> if there is a reference URI.
		 */
		 ...
		 * <p>
		 * This version returns default string representation if there is a reference URI; otherwise, the node ID is returned.
		 * </p>
